...
<aside id="adminSidebar" class="w-64 bg-slate-900 text-slate-300 flex flex-col border-r border-slate-800 transition-all duration-300 fixed md:sticky md:top-0 z-30 h-screen -translate-x-full md:translate-x-0 shrink-0">
    ...
    <div class="p-3 border-t border-slate-800 bg-slate-950/40">
           ...
           title="Acceder a Mi Perfil de Usuario">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-bold text-white truncate group-hover:text-primary-container transition-colors"><?= e($_au['name'] ?? 'Administrador') ?></p>
                ...
                <div class="flex items-center gap-1.5 mt-1">
                    ...
                    <span class="text-[10px] text-slate-400 capitalize truncate"><?= e($_au['role'] ?? 'admin') ?></span>
                </div>
            </div>
        </a>
        <div class="mt-2.5 pt-2 border-t border-slate-800/60 flex items-center justify-between px-1 text-xs text-slate-400">
            ...
            <a href="/admin/logout" onclick="return confirmarCierreSesion(event, this.href);" class="text-red-400 hover:text-red-300 hover:bg-red-950/50 px-2 py-1 rounded-lg transition-colors flex items-center gap-1 font-semibold text-[11px]" title="Cerrar Sesión">
